...
def test_provider_routes_require_authentication_and_never_echo_credentials(monkeypatch) -> None:
    ...
    monkeypatch.setattr(provider_api, "create_provider", lambda _: OfflineProvider())
    with TestClient(app) as client:
        assert client.get("/api/provider/status").status_code == 401
        token = _access_token(client)
        headers = {"Authorization": f"Bearer {token}"}
        secret = "never-return-this-provider-secret"
        configured = client.post("/api/provider/configure", headers=headers, json={"provider": "IBKR", "credentials": {"api_key": "id", "secret": secret}})
        ...
        assert client.post("/api/provider/configure", headers=headers, json={"provider": "IBKR", "unknown": True}).status_code == 422
