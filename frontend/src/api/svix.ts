...
import type { Components, MarketStatus, SVIXPoint } from '../types'
...
export const getMarketStatus = async () => (await api.get<MarketStatus>('/svix/market-status')).data
export const getIntradaySVIX = async (sessionDate?: string) => (await api.get<SVIXPoint[]>('/svix/intraday', { params: sessionDate ? { session_date: sessionDate } : {} })).data
